...
export default async function GenericPage({ params }: { params: Promise<{ slug: string[] }> }) {
  ...
  try {
    ...
    return (
      <div className="min-h-screen bg-primary-950">
        <Header />
        <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-12 pt-28">
          <ErrorBoundary>
            <article className="bg-primary-900 border border-primary-800 rounded-lg shadow-sm overflow-hidden">
              {image && (
                <ResponsiveImage
                  image={image}
                  alt={image.alt || title}
              ...
              <div className="p-6 md:p-8">
                <h1 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-display font-normal text-white mb-4 md:mb-6 tracking-wider uppercase">{title}</h1>
                <div className="prose prose-invert prose-headings:text-white prose-p:text-gray-300 prose-a:text-accent-500 prose-strong:text-white prose-sm sm:prose lg:prose-lg max-w-none" dangerouslySetInnerHTML={{ __html: bodyHtml }} />
              </div>
            </article>
          </ErrorBoundary>
        </main>
