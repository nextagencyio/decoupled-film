...
import { GET_PROJECT_BY_PATH } from '@/lib/queries'
...
interface ProjectByPathData {
  route: {
    entity: DrupalProject
  } | null
}

async function getProject(path: string): Promise<DrupalProject | null> {
  try {
    const requestHeaders = await headers()
    const apolloClient = getServerApolloClient(requestHeaders)
    const { data } = await apolloClient.query<ProjectByPathData>({
      query: GET_PROJECT_BY_PATH,
      variables: { path },
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const item = await getProject(path)

  if (!item) {
    return { title: 'Project Not Found | Film' }
  }
                ...
                <dl className="space-y-4">
                  {(item as any).director && (
                    <div>
                      ...
                      <dd className="font-semibold text-gray-900">{(item as any).director}</dd>
                    </div>
                  )}
                  {(item as any).releaseYear && (
                    <div>
                      <dt className="text-sm text-gray-500">Release Year</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).releaseYear}</dd>
                    </div>
                  )}
                  {(item as any).runtime && (
                    <div>
                      <dt className="text-sm text-gray-500">Runtime</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).runtime}</dd>
                    </div>
                  )}
                  {(item as any).trailerUrl && (
                    <div>
                      <dt className="text-sm text-gray-500">Trailer URL</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).trailerUrl}</dd>
                    </div>
                  )}
                </dl>
                <div className="mt-8">
